...
function App() {
  ...
  // ── budget — localStorage persisted ────────────────────────
  const [budget, setBudget] = useState(() => {
    const saved = lsGet('familyTrip_budget', null);
    if (saved) return saved;
    // migrate old key
    try {
      const old = localStorage.getItem('trip_budget_perPerson');
      if (old) return { perPerson: parseInt(old, 10) || 60000, memberCount: 7 };
    } catch(e) {}
    return { perPerson: 60000, memberCount: 7 };
  });
  useEffect(() => { lsSet('familyTrip_budget', budget); }, [budget]);

  const [showBudgetForm, setShowBudgetForm] = useState(false);
  const handleSaveBudget = (b) => { setBudget(b); setShowBudgetForm(false); };
  ...
  const topBarFor = (t) => ({
    home:      { sub: '2026 / 12 / 31 · 週三', title: '哈囉，週三早安 👋' },
  ...
  if (tab === 'home') {
    ...
    content = <ScreenToday expenses={expenses} onAddExpense={()=>setShowAdd(true)}/>;
    ...
    content = <ScreenExpense expenses={expenses} onAddExpense={()=>setShowAdd(true)} onViewSplit={()=>setShowSplit(true)} onEditExpense={handleEditExpense} budget={budget} onEditBudget={()=>setShowBudgetForm(true)}/>;
  } else if (tab === 'stay') {
    content = <ScreenStay stays={stays} onEditStay={handleEditStay} onDeleteStay={handleDeleteStay}/>;
  }

  const showFab = tab === 'today' || tab === 'itinerary' || tab === 'expense' || tab === 'stay';
  const fabIsItinerary = tab === 'today' || tab === 'itinerary';
