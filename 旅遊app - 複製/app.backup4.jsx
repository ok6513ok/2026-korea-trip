...
function App() {
  ...
  let content;
  if (tab === 'home') {
    content = <ScreenDashboard expenses={expenses} memberBudgets={memberBudgets} onNav={handleNav} onAddExpense={()=>setShowAdd(true)} todayItems={allItinItems[TRIP.todayIndex] || []}/>;
  } else if (tab === 'today') {
    ...
    <div style={{
      ...
      {showItinForm && (
        <ItineraryForm
          ...
          onSave={(item) => {
            const dayIdx = item.dayIdx ?? (tab === 'itinerary' ? itineraryDay : TRIP.todayIndex);
