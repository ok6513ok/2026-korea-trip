...
function calcMemberShare(memberId, expenses) {
  return expenses.reduce((sum, e) => {
    if (!e.split.includes(memberId)) return sum;
    return sum + e.krw / e.split.length;
  }, 0);
}
...
// Expense (消費記錄)
// ─────────────────────────────────────────────────────────────
function ScreenExpense({ expenses, onAddExpense, onViewSplit, onEditExpense, budget, onEditBudget }) {
  ...
  // ── 英雄卡片資料 ──────────────────────────────────────────
  const perPerson   = budget ? budget.perPerson : 60000;
  const totalBudget = budget ? budget.perPerson * budget.memberCount : TRIP.budget.totalTWD;
  ...
  if (memberView === 'all') {
    ...
    heroBudget   = perPerson;
    heroSubtitle = '應分攤金額';
  }
  const pct = Math.min(100, (heroSpent / heroBudget) * 100);
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {memberView === 'all' && onEditBudget && (
              <button onClick={onEditBudget} style={{
...
// ─────────────────────────────────────────────────────────────
function ScreenAddExpense({ onClose, onSave, editExpense }) {
  ...
  const [date, setDate] = useState(editExpense?.date || TRIP.todayLabel);
  ...
  const save = () => {
    onSave({
      ...
      day: editExpense?.day ?? TRIP.todayIndex,
      date: editExpense?.date || TRIP.todayLabel,
      item: name || '未命名消費',
      cat, krw, payer,
      split: splitIds,
      note,
      ...
      <div style={{
        ...
        <Card pad={0}>
          ...
          <FieldRow label="日期" icon="today">
            <span style={{ fontSize: 14, color: T.ink, fontWeight: 600 }}>{date}</span>
          </FieldRow>
        </Card>
...
// ─────────────────────────────────────────────────────────────
// BudgetForm — 修改預算 overlay
// ─────────────────────────────────────────────────────────────
function BudgetForm({ budget, onClose, onSave, expenses }) {
  const [perPerson, setPerPerson] = useState(String(budget.perPerson));

  const MEMBER_COUNT = 7;
  const pp    = parseInt(perPerson.replace(/[^\d]/g, '')) || 0;
  const total = pp * MEMBER_COUNT;
  const totalSpentTWD = expenses ? krwToTwd(expenses.reduce((s, e) => s + e.krw, 0)) : 0;
  const myShareTWD = totalSpentTWD / MEMBER_COUNT;
  ...
  const BRow = ({ label, icon, children }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px' }}>
      <Icon name={icon} size={18} color={T.ink3}/>
      <span style={{ fontSize: 13, color: T.ink2, fontWeight: 600, flexShrink: 0, minWidth: 72 }}>
        {label}
      </span>
      <div style={{ flex: 1, minWidth: 0 }}>{children}</div>
    </div>
  ...
  const BDivider = () => (
    <div style={{ height: 1, background: T.divider, marginLeft: 44 }}/>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={() => onSave({ perPerson: pp || 60000, memberCount: MEMBER_COUNT })} style={{
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '4px 16px 40px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {/* 說明 */}
        <div style={{
          background: T.primarySoft, borderRadius: 16, padding: '12px 16px',
          fontSize: 13, color: T.primary, lineHeight: 1.6,
        }}>
          設定個人旅行預算。個人旅程總金額為目前應分攤花費，僅供參考。
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 18, overflow: 'hidden', boxShadow: T.shadowCard }}>
          ...
          <BDivider/>
          <BRow label="成員人數" icon="split">
            <div style={{ textAlign: 'right', fontSize: 15, fontWeight: 700, color: T.ink2 }}>
              {MEMBER_COUNT} 人
            </div>
          </BRow>
          <BDivider/>
          <BRow label="個人旅程總金額" icon="wallet">
            <div style={{
              textAlign: 'right',
              fontSize: 15, fontWeight: 700, color: T.ink2,
        ...
        {/* 快速選項 */}
        <div>
          <div style={{ fontSize: 12, color: T.ink3, fontWeight: 600, marginBottom: 8, paddingLeft: 4 }}>
            快速設定每人預算
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[30000, 50000, 60000, 80000, 100000].map(v => (
              <button key={v} onClick={() => setPerPerson(String(v))} style={{
                border: pp === v ? 'none' : '1px solid ' + T.divider,
                background: pp === v ? T.primary : '#fff',
                color: pp === v ? '#fff' : T.ink2,
                padding: '6px 14px', borderRadius: 999,
                fontFamily: T.font, fontSize: 12, fontWeight: 700,
                cursor: 'pointer',
              }}>NT$ {v.toLocaleString()}</button>
            ))}
          </div>
        </div>
      </div>
